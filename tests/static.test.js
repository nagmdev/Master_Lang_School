...
test('shared modules are underscore-prefixed so Vercel does not expose them', () => {
  ...
  const exposed = apiFiles.filter(f => !f.startsWith('_') &&
    !['applications', 'application', 'login', 'logout', 'session', 'file', 'careers', 'career', 'careerfile', 'jobs', 'contact'].includes(f.replace(/\.js$/, '')));
...
/* ------------- 13. Jobs — seed sync, pages & generator conformance ------------- */
group('13. Jobs — seed sync, pages & generator conformance');

// careers.js is the source of truth the sync tool regenerates the seed from.
const sharedSrc = fs.readFileSync(path.join(ROOT, 'careers.js'), 'utf8');
const seedFile = fs.readFileSync(path.join(ROOT, 'api', '_lib', 'jobs.seed.js'), 'utf8');

function seedJobsFromFile() {
  const m = seedFile.match(/var SEED_JOBS = (\[[\s\S]*?\]);/);
  assert(m, 'SEED_JOBS literal not found in jobs.seed.js');
  return Function('return ' + m[1])();
}
function eq(a, b, m) {
  if (a !== b) throw new Error((m || 'expected') + `: got ${JSON.stringify(a)} ≠ ${JSON.stringify(b)}`);
}
function sharedSlugs() {
  const m = sharedSrc.match(/^  var SLUGS = \[([\s\S]*?)\];$/m);
  assert(m, 'SLUGS literal not found in careers.js');
  return m[1].split(',').map((s) => s.trim().replace(/['"]/g, '')).filter(Boolean);
}

test('jobs.seed.js is byte-synced with careers.js (source of truth)', () => {
  assertHas(seedFile, 'GENERATED BY tools/sync-jobs-seed.js', 'seed lost its generation marker');
  const slugs = sharedSlugs();
  const jobs = seedJobsFromFile();
  assert(slugs.length === jobs.length, `${slugs.length} slugs vs ${jobs.length} seeds`);
  for (let i = 0; i < jobs.length; i++) {
    eq(jobs[i].id, slugs[i], 'seed[' + i + '].id != SLUGS[' + i + ']');
    eq(jobs[i].order, i, 'seed[' + i + '] order out of step');
    assert(jobs[i].en && jobs[i].en.r && jobs[i].ar && jobs[i].ar.r, 'seed[' + i + '] lacks en/ar r');
  }
});

test('every seed job has its own job page and apply page on disk', () => {
  for (const j of seedJobsFromFile()) {
    for (const f of [j.id + '.html', 'apply-' + j.id + '.html']) {
      const page = fs.readFileSync(path.join(ROOT, f), 'utf8');
      assertHas(page, '<title>', f + ' missing title');
      assertHas(page, 'static cfg = { mode: "' + (f.startsWith('apply-') ? 'apply' : 'job') + '", slug: "' + j.id + '"', f + ' wrong cfg');
    }
  }
});

test('job pages gate the Apply CTA behind careerOpen; apply pages carry jobId + closed notice', () => {
  for (const f of fs.readdirSync(ROOT).filter(n => /^[a-z0-9-]+\.html$/.test(n) && !n.startsWith('apply-') && n !== 'careers.html' && n !== 'index.html')) {
    const page = fs.readFileSync(path.join(ROOT, f), 'utf8');
    if (!page.includes('data-dc-script')) continue;
    assertHas(page, '<sc-if value="{{ careerOpen }}" hint-placeholder-val="{{ true }}">', f + ' CTA not guarded');
    assertHas(page, '<sc-if value="{{ careerClosed }}" hint-placeholder-val="{{ false }}">', f + ' closed notice missing');
    assertHas(page, '{{ careerClosedNotice }}', f + ' notice text binding missing');
  }
  for (const f of fs.readdirSync(ROOT).filter(n => /^apply-[a-z0-9-]+\.html$/.test(n))) {
    const page = fs.readFileSync(path.join(ROOT, f), 'utf8');
    assertHas(page, '<input type="hidden" name="jobId" value="{{ jobIdKey }}">', f + ' jobId hidden field missing');
    assertHas(page, '<input type="hidden" name="position" value="{{ positionKey }}">', f + ' position hidden field missing');
    assertHas(page, '<sc-if value="{{ careerClosed }}" hint-placeholder-val="{{ false }}">', f + ' closed notice missing');
  }
});

test('apply pages use a clean em dash in their titles (mojibake regression)', () => {
  for (const f of fs.readdirSync(ROOT).filter(n => /^apply-/.test(n) && n.endsWith('.html'))) {
    const page = fs.readFileSync(path.join(ROOT, f), 'utf8');
    const m = page.match(/<title>([^<]*)<\/title>/);
    assert(m, f + ' title missing');
    assert(m[1].indexOf('â€') === -1 && m[1].indexOf('\uFFFD') === -1 && m[1].includes('—'),
      f + ' title still mojibake: ' + JSON.stringify(m[1]));
  }
});

test('careers.js exposes the live-jobs sync hooks used by every careers page', () => {
  assertHas(sharedSrc, 'jobsEndpoint', 'careers.js lacks jobsEndpoint');
  assertHas(sharedSrc, 'fetchJobs', 'careers.js lacks fetchJobs');
  assertHas(sharedSrc, 'liveData()', 'careers.js lacks liveData');
  assertHas(sharedSrc, 'embeddedJob', 'careers.js lacks the MS_JOB fallback');
  assertHas(sharedSrc, 'careerOpen()', 'careers.js lacks careerOpen');
  assertHas(sharedSrc, 'careers.closed || \'\'', 'careers.js lacks the closed notice binding');
  assertHas(sharedSrc, 'window.MS_JOB', 'careers.js lacks MS_JOB handling');
  assertHas(sharedSrc, 'careerSendError: this.content()[this.state.lang].careers.closed', 'submitCareer has no closed guard');
  assertHas(sharedSrc, 'careersRows', 'careers.js renderVals lacks careersRows');
});

test('the careers list page iterates careersRows (live or dictionary fallback)', () => {
  const page = fs.readFileSync(path.join(ROOT, 'careers.html'), 'utf8');
  assertHas(page, '{{ careersRows }}', 'careers.html does not iterate careersRows');
});

test('non-seed jobs that exist in the store generate pages with MS_JOB + slug-only cfg', () => {
  const gen = require(path.join(ROOT, 'tools', 'generate-job-pages.js'));
  const tpl = fs.readFileSync(path.join(ROOT, 'german-teacher.html'), 'utf8');
  const applyTpl = fs.readFileSync(path.join(ROOT, 'apply-german-teacher.html'), 'utf8');
  const job = {
    id: 'part-time-librarian',
    en: { r: 'Part-Time Librarian', key: 'Librarian', d: 'Library', ty: 'Part-time', desc: 'Curate the school library.', resp: ['x'], qual: ['y'] },
    ar: { r: 'أمين مكتبة', key: 'أمين مكتبة', d: 'مكتبة', ty: 'دوام جزئي', desc: 'تنظيم مكتبة المدرسة', resp: ['س'], qual: ['ق'] },
  };
  const html = gen.renderJobPage(tpl, job, undefined);
  const apply = gen.renderApplyPage(applyTpl, job, undefined);
  assertHas(html, 'window.MS_JOB =', 'generated job page lacks the embedded payload');
  assertHas(html, '"id":"' + job.id + '"', 'generated job payload lacks the id');
  assertHas(html, 'static cfg = { mode: "job", slug: "' + job.id + '" }', 'generated job page should not use an index');
  assertHas(html, '<title>' + job.en.r + ' — Careers | Masters Language School</title>', 'generated title wrong');
  assertHas(apply, '<input type="hidden" name="jobId" value="{{ jobIdKey }}">', 'generated apply page missing jobId');
  // canonical jobs keep byte-identity: an index cfg, no payload
  const seedJob = seedJobsFromFile()[0];
  const canonical = gen.renderJobPage(tpl, seedJob, sharedSlugs().indexOf(seedJob.id));
  assert(!canonical.includes('window.MS_JOB ='), 'canonical seed job page must not embed the payload');
  assertHas(canonical, 'static cfg = { mode: "job", slug: "' + seedJob.id + '", index: 0 }', 'canonical cfg');
});

test('the generator toolbox exports the expected renderers', () => {
  const gen = require(path.join(ROOT, 'tools', 'generate-job-pages.js'));
  for (const k of ['renderJobPage', 'renderApplyPage', 'jobMeta', 'payloadScript', 'readArgs']) {
    assert(typeof gen[k] === 'function', 'generator does not export ' + k);
  }
});
